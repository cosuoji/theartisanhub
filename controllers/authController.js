...
import { setCookies, generateTokens } from "../utils/authHelpers.js";
...
export const logout = asyncHandler(async (req, res) => {
  try {
    ...
    if (accessToken) {
      ...
      // Save blacklisted token with expiration matching its expiry
      await Blacklist.create({
        jti: decoded.jti,
        expiresAt: new Date(decoded.exp * 1000), // Convert exp (seconds) → ms
      });
    }
...
try {
 ...
 export const refreshToken = asyncHandler(async (req, res) => {
  try {
    ...
    // 2️⃣ Generate and save new tokens
    const { accessToken, refreshToken: newRefreshToken } = generateTokens(user._id);
    user.refreshToken = newRefreshToken;
    await user.save();

    // 3️⃣ Send back new cookies
